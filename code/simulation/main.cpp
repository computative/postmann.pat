...
void observational(double * observables , mat rpp, mat Dpluspp, mat Dminuspp, mat invDpluspp, mat invDminuspp, double a, double b, mat c, double w);
...
int main(int nargs, char *args[])
{
    ...
    // setting number of iterations pointer to physics-observables
    int iterations = pow(2,20);
    double * observables = new double [4];
    for (int j = 0; j < 4; j++)
        observables[j] = 0;

    // execute vmc
    vmc(observables, observational, x[0], x[1], c, w, dt, iterations);

    // gathering results and printing to terminal
    double * globalsum = new double [5];
    MPI_Reduce(observables, globalsum, 5, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    if (my_rank == 0) {
        cout.precision(7);
        cout << globalsum[0]/(numprocs*iterations) << " " << globalsum[1]/(numprocs*iterations)
             << " " << globalsum[2]/(numprocs*iterations) << " " << globalsum[3]/(numprocs*iterations) << endl;
    }
    MPI_Finalize();
    return 0;
}
...
void vmc(double * observables, void (*sample)(double *, mat, mat , mat, mat , mat, double, double, mat, double), double a,double b, mat c, double w, double dt, int iterations) {
    ...
    for (int u = 0; u < iterations; u++) {
        ...
        if (s % 2 == 0) {
            Dplus  = D(r, 0, a, w);
            if (abs(det(Dplus)) < eps) {
                cout << abs(det(Dplus)) << endl;
                continue;
            }
            invDplus = inv(Dplus);
        } else {
            Dminus = D(r, 1, a, w);
            if ( abs(det(Dminus)) < eps) {
                cout << abs(det(Dminus)) << endl;
                continue;
            }
        ...
        double * observablespp = new double[4];
        for (int k = 0; k<4; k++)
            observablespp[k] = observables[k];
