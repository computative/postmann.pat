...
int main(int argc, char *argv[])
{
    ...
    int k = 0;
    while (i < iterations ) {
        r1 = r1_old + 0.001*2*0.5*gradPsi(r1_old, r2_old, a, b, c, w) + randn<vec>(2)*sqrt(0.001);
        r2 = r2_old + 0.001*2*0.5*gradPsi(r1_old, r2_old, a, b, c, w) + randn<vec>(2)*sqrt(0.001);
//        r1 = r1_old + randu<vec>(2) - 0.5;
  //      r2 = r2_old + randu<vec>(2) - 0.5;
        wf = psi_T(r1,r2, a,b,c,w);
        if ( wf*wf*G(r1, r2,r1_old,r2_old,a,b,c,w)/(wf_old*wf_old*G(r1_old,r2_old,r1,r2,a,b,c,w)) > rand_double(gen) ) {
...
vec gradPsi(vec r1,vec r2, double a, double b, double c, double w) {
    ...
    double num = 0;//c*(r1[0] - r2[0] + r1[1] - r2[1] )/( (1 + b*rij)*(1 + b*rij)*rij );
