...
int main(int nargs, char *args[])
{
    ...
    // init steepest decent
    int iterations = pow(2,16);
    vec delFp = zeros<vec>(2);
    vec delFpp = 0.01*randn<vec>(2);
    int s = 0;

    // infinite loop, break inside loop
    while(true) {
        // make vmc run at for given iterations
        ...
        // compute new variational params and decide step optimal decent step
        ...
        // print currect variational params for survey
        if (my_rank == 0) {
            cout.precision(7);
            cout << "(alpha,beta) = " << endl;
            x.raw_print();
            cout << "E = " << globalsum[0]/(numprocs*iterations) << endl << endl;
        }

        // if converged, break infinite loop
